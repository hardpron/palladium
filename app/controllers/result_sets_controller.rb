class ResultSetsController < ApplicationController
  ...
  def show
    @product = Product.find(product_find_by_id)
  ...
  def update
    ...
    respond_to do |format|
      if @result_sets.update_all(result_set_params)
        format.html { redirect_to product_plan_run_result_sets_path(product_find_by_id, set_plan, set_run), notice: 'Result set was successfully updated.' }
        # This string will be commented because creation can be only through API
